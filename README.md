...
8. **Coordinate display**: An opt-in deviation — pressing P toggles the ship's coordinates in the score bar (e.g. `X 123.4 Y 5.2 Z 45.6`). X and Z wrap with the 256-tile periodic world so the launchpad always reads as tiles 0-8 on both axes; Y is the raw altitude (positive down, negative up) and never wraps. Off by default
